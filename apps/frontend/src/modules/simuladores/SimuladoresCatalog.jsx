...
import { simulatorsApi } from '../actividades/api/simulatorApi'
...
export default function SimuladoresCatalog({ onlyActive = false, showStatus = false }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(async () => {
      try {
        const data = await simulatorsApi.findAll(onlyActive)
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 bg-slate-50 px-5 py-4">
              <div className="min-w-0">
                ...
                {showStatus && <p className="mt-0.5 text-xs text-slate-400">ID {sim.idSimulador}</p>}
              </div>
            ...
            <div className="flex flex-1 flex-col gap-4 px-5 py-4">
              ...
              <div className="mt-auto flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                {showStatus ? (
                  ...
                  <span className="inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold bg-sky-100 text-sky-700">
